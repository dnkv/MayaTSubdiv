...
bool init_KVCArray_from_MatlabVar(KVCObject *kvco, MatlabVar *mv)
{
    ...
    {
        KVCComplexArray<C> *ka = dynamic_cast<KVCComplexArray<C> *>(kvco); if (!ka) return false;
    ...
    KVCRealArray<C> *ka = dynamic_cast<KVCRealArray<C> *>(kvco);
    if (ka)
    {
        init_KVCArray_from_MatlabVar(ka, mv);
        ka->v.resize(ka->size());
    ...
    KVCIndexArray<C> *kmia = dynamic_cast<KVCIndexArray<C> *>(kvco);
    if (kmia)
    {
        init_KVCArray_from_MatlabVar(kmia, mv);
        kmia->v.resize(kmia->size());
        for (size_t k=0; k<kmia->size(); k++)
        {
...
auto_ptr<mwSize> buildMWSizeDims(KVCArray &ka)
{
    if (ka.nDim()==0) return auto_ptr<mwSize>(0);
    mwSize *dims = new mwSize[ka.nDim()];
    for (size_t k=0; k<ka.nDim(); k++)
    {
        dims[k] = (mwSize)ka.dim(k);
    }
...
inline bool create_MatlabVar_from_KVCRealArray(MatVarType mt,  KVCRealArray<C> *ka)
{
    std::vector<mwSize> dims; for (size_t k=0; k<ka->nDim(); k++) dims.push_back((mwSize)ka->dim(k)); // convert to mwSize
...
inline bool create_MatlabVar_from_KVCIndexArray(MatVarType mt,  KVCIndexArray<C> *ka)
{
    std::vector<mwSize> dims; for (size_t k=0; k<ka->nDim(); k++) dims.push_back((mwSize)ka->dim(k));
...
{
    KVCRealArray<C> *kmra = dynamic_cast<KVCRealArray<C> *>(ka);
    if (kmra) return create_MatlabVar_from_KVCRealArray(mt, kmra);
    
    KVCIndexArray<C> *kmia = dynamic_cast<KVCIndexArray<C> *>(ka);
    if (kmia) return create_MatlabVar_from_KVCIndexArray(mt, kmia);
    
    KVCComplexArray<C> *kmca = dynamic_cast<KVCComplexArray<C> *>(ka);
    if (kmca) return create_MatlabVar_from_KVCComplexArray(mt, kmca);
...
bool create_MatlabVar_from_KVCCell(KVCCellBase *kc)
{
    ...
    MatlabVar *mv = new MatlabVar(MAT_CELL, (mwSize)kc->nDim(), buildMWSizeDims(*kc).get());
